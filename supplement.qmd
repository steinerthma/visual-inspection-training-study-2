---
title: "Supplement - Visual analysis - Training study"
keywords:
  - Supplement
abstract: |
...
if (!packageDate("wmisc") >= as.Date("2024-06-18")) devtools::install_github("jazznbass/wmisc")
library(wmisc)
...
The majority of the data where collected at the University of Potsdam at the end of a lecture on inclusive education.
...
nice_frequencies(dat_subjects$mathe)

dat_subjects |> select(1:16, -rt, -gesch, -age) |> 
...
data.frame(
  ...
  nice_table(title = "Contrasts for interactions of time by condition", file = TRUE)
```
...
data.frame(
  ...
  nice_table(title = "Contrasts for interaction of time by condition for confidence of judgement ratings", file = TRUE)
```
